...
export default function EditBlog() {
  ...
  useEffect(() => {
    useCheckAuthToken();
    if (sessionStorage.getItem("blogEdit")) {
      var data = JSON.parse(sessionStorage.getItem("blogEdit"));
      console.log(sessionStorage.getItem("blogEdit"))
      id.current = data.id;
      heading.current.value = data.title;
      desc.current.value = data.desc;
      blog.current.value = data.content.replace(/\ /g, " ");
  ...
  function saveLocally() {
    ...
    try {
      ...
      sessionStorage.setItem("blogEdit", JSON.stringify(jsonFile));
    } catch (e) {}
  ...
  function saveBlog(status) {
    ...
    axios
      .post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/blog/edit-blog`,
        jsonFile,
        {
          headers: {
      ...
      .then((res) => {
        ...
        if (status == 1) text = "Blog Updated Successfully";
